...
const Search = () => {
    ...
    const { provinces, areas, prices, categories, dictricts } = useSelector(state => state.app)
    ...
    const handleSearch = () => {
        ...
        let titleSearch = `${queryTextObj.category
            ...
            : 'Cho thuê tất cả'} ${queryTextObj.province
                ...
                : ''} ${queryTextObj.dictrict
                    ? `huyện ${queryTextObj.dictrict}`
                    : ''} ${queryTextObj.price
    ...
    return (
        <>
            <div className='p-[10px] w-4/5 my-3 bg-[#febb02] rounded-lg flex-col lg:flex-row flex items-center justify-around gap-2' >
                ...
                <span onClick={() => handleShowModal(dictricts, 'dictrict', 'Huyện')} className='cursor-pointer flex-1'>
                    <SearchItem IconBefore={<HiOutlineLocationMarker />} IconAfter={<BsChevronRight color='rgb(156, 163, 175)' />} text={queries.dictrict} defaultText={'Huyện'} />
                </span>
